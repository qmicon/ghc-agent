...
async def generate_offer_plan(shopify_url):
    ...
    2. All required sections and snippets
    3. Detailed implementation instructions for each file
    4. Actual code blocks for each file that follows the implementation instructions

    Required File Structure:
    - templates/page.offer.json (main template file that defines sections)
    - sections/*.liquid (section files that implement functionality, including their own JS and CSS)
    - snippets/*.liquid (reusable components used by sections)
    ...
    Required File Structure:
    1. templates/page.offer.json
       - Must define all sections in the correct order
       - Must include section settings
       - Must be valid JSON format

    2. sections/*.liquid
       - Each section must have its own .liquid file
       - Must include schema for settings
       - Must include its own JavaScript and CSS directly in the file
       - JavaScript should be wrapped in <script> tags
       - CSS should be wrapped in <style> tags
       - Can use snippets for reusable components
       - Must be valid Liquid syntax

    3. snippets/*.liquid
       - Reusable components used by sections
       - Must be valid Liquid syntax
    ...
    ---
    FILE: templates/page.offer.json
    TYPE: json
    CONTENT:
    ```json
    {{
    ...
    2. JSON templates under templates/ only need to include sections and order, but sections should have some settings according to the requirements.
